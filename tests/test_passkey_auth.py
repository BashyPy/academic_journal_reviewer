...
async def test_generate_registration_options():
    ...
    with patch.object(
        webauthn_service, "generate_registration_options", new_callable=AsyncMock
    ) as mock_gen:
...
async def test_verify_registration():
    ...
    with patch.object(
        webauthn_service, "verify_registration", new_callable=AsyncMock
    ) as mock_verify:
        mock_verify.return_value = True

        credential = {
            "id": "test_credential_id",
            "response": {"publicKey": "test_public_key", "counter": 0, "transports": ["internal"]},
        }
...
async def test_generate_authentication_options():
    ...
    with patch.object(
        webauthn_service, "generate_authentication_options", new_callable=AsyncMock
    ) as mock_gen:
        mock_gen.return_value = {
...
async def test_verify_authentication():
    ...
    with patch.object(
        webauthn_service, "verify_authentication", new_callable=AsyncMock
    ) as mock_verify:
