...
async def test_api_upload_flow():
    ...
    with (
        patch("app.api.routes.document_parser") as mock_parser,
...
async def test_api_get_report():
    ...
    with patch("app.api.routes.mongodb_service") as mock_db:
        mock_db.get_submission = AsyncMock(
            return_value={"_id": "test", "status": "completed", "final_report": "Report"}
        )
...
async def test_auth_login():
    ...
    with patch("app.api.auth_routes.user_service") as mock_user:
        mock_user.authenticate_user = AsyncMock(
            return_value={"email": "test@test.com", "api_key": "key123"}
        )
...
def test_middleware_permissions():
    ...
    assert check_permission({"role": "admin"}, "admin") is True
...
def test_middleware_waf():
    ...
    ok, _msg = waf.scan_request(
        Mock(url=Mock(__str__=lambda x: "/api"), headers={}, method="GET"), ""
    )
    assert ok is True
...
async def test_user_authenticate():
    ...
    service = UserService()
    with (
        patch.object(service, "users_collection") as mock_coll,
...
def test_otp_verify():
    ...
    service = OTPService()
    otp = service.generate_otp("test@test.com")
    assert service.verify_otp("test@test.com", otp) is True
