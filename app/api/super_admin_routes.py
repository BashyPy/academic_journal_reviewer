...
async def list_all_submissions(
    ...
    submissions = await db.submissions.find(query).skip(skip).limit(limit).to_list(length=limit)
...
async def get_user_activity(admin: dict = Depends(require_super_admin), days: int = 7):
