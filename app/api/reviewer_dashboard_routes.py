...
def require_reviewer(user: dict = Depends(get_current_user)):
    ...
    if role not in [UserRole.REVIEWER.value, UserRole.EDITOR.value, UserRole.ADMIN.value, UserRole.SUPER_ADMIN.value]:
...
async def get_reviewer_stats(user: dict = Depends(require_reviewer)):
    ...
    pending = await db.review_assignments.count_documents({"reviewer_id": reviewer_id, "status": "pending"})
    in_progress = await db.review_assignments.count_documents({"reviewer_id": reviewer_id, "status": "in_progress"})
    completed = await db.review_assignments.count_documents({"reviewer_id": reviewer_id, "status": "completed"})
    
    # Calculate average review time
    pipeline = [
        {"$match": {"reviewer_id": reviewer_id, "status": "completed", "completed_at": {"$exists": True}}},
        {
            "$project": {
                "review_time": {"$subtract": ["$completed_at", "$started_at"]}
            }
        },
        {
            "$group": {
...
async def get_reviewer_assignments(
    ...
    assignments = await db.review_assignments.find(query).sort("assigned_at", -1).skip(skip).limit(limit).to_list(length=limit)
...
async def get_assignment_detail(assignment_id: str, user: dict = Depends(require_reviewer)):
    ...
    assignment = await db.review_assignments.find_one({"_id": ObjectId(assignment_id), "reviewer_id": reviewer_id})
...
async def get_review_domains(user: dict = Depends(require_reviewer)):
    ...
    # Get all assignments
    assignments = await db.review_assignments.find({"reviewer_id": reviewer_id}).to_list(length=1000)
