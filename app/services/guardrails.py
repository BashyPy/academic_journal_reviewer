...
class AcademicGuardrails:
    def __init__(self):
        self.ethical_keywords = [
            "plagiarism", "fabrication", "falsification", "misconduct",
            "duplicate", "self-plagiarism", "ghost author", "gift author"
        ]
    ...
    def sanitize_content(self, content: str, violations: List[GuardrailViolation]) -> str:
        ...
        for violation in violations:
            if violation.action == "sanitize" and violation.type == "unprofessional_tone":
                ...
                for bad_term, good_term in replacements.items():
                    sanitized = re.sub(bad_term, good_term, sanitized, flags=re.IGNORECASE)
